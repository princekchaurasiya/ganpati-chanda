...
- ✅ **Multi-year scoping (2026-02-15)**: Har transaction endpoint (`/chanda`, `/expenses`, `/transfers`, `/reimbursements`, `/event-transfers`, `/dashboard`, `/members/summary`, `/members/{name}`) ab optional `?year=YYYY` query param accept karta hai — regex `date` field pe filter karta hai. New `GET /api/years` endpoint distinct years return karta hai (always includes current year). Frontend axios interceptor auto-appends `year` to all GETs from localStorage `chanda_active_year`. Layout header pe **Calendar icon + year chip** — tap karke bottom sheet me year switcher (All years / 2026 / 2027 …). Year change pe `window.location.reload()` all data refreshes cleanly. New year (2027) me app shuru se khaali dikhta hai — old 2026 data untouched.
