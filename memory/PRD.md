...
- ✅ **Batch #2 data + Add Member on Members page (2026-09-11)**: 26 more chandas imported across Book 2 (95-100) and new **Book 4** (151-170), all dated 2026-09-11 with the confirmed defaults (Row 99 = Pending, all default mode = Cash, `pravin bhai` merged into `pravin`, Book 4 = B4 151-200 Unassigned). 5 new collectors auto-added (mukesh, prince, pravin, amar, dinesh chaurasiya). Members page now has an **Add Member** button + inline modal (Enter=save, Esc=close, disabled while saving). Backend POST /api/collectors now returns **400** on duplicate names so the UI can show a clear error toast. Pooja Iyer restored to seed. Total collectors = 11.
